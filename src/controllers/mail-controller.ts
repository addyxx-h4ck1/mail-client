...
export const mailController = async (req: Request, res: Response) => {
  ...
  if (!existUser) return res.status(404).json({ err: 'user not found' });
  //google auth
  ...
  oAuth2Client.setCredentials({
    ...
    scope: existUser.scope as string,
    token_type: existUser.tokenType,
  });
  ...
  async function sendMail() {
    try {
      ...
      const mailOptions = {
        from: `smtps Mailer <${existUser?.email}>`, // this is the authenticated user email
